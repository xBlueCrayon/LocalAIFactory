...
public static class ApiEndpoints
{
    public static void Map(WebApplication app)
    {
        ...
        api.MapGet("/catalog/paymentterms", (ErpDbContext db) => Results.Ok(db.Set<PaymentTerm>().OrderByDescending(x => x.Id).ToList()));
        api.MapPost("/catalog/paymentterms", ([Microsoft.AspNetCore.Mvc.FromServices] CatalogCrudService<PaymentTerm> svc, [Microsoft.AspNetCore.Mvc.FromBody] PaymentTerm e) => { try { return Results.Created("/api/catalog/paymentterms", svc.Create(e)); } catch (LafErp.Core.DomainException ex) { return Results.BadRequest(new { error = ex.Message }); } });
        api.MapGet("/catalog/taxcodes", (ErpDbContext db) => Results.Ok(db.Set<TaxCode>().OrderByDescending(x => x.Id).ToList()));
        api.MapPost("/catalog/taxcodes", ([Microsoft.AspNetCore.Mvc.FromServices] CatalogCrudService<TaxCode> svc, [Microsoft.AspNetCore.Mvc.FromBody] TaxCode e) => { try { return Results.Created("/api/catalog/taxcodes", svc.Create(e)); } catch (LafErp.Core.DomainException ex) { return Results.BadRequest(new { error = ex.Message }); } });
        api.MapGet("/health"
        , () => Results.Ok(new { status = "ok", product = "LAF Enterprise ERP V2" }));
    }
}
