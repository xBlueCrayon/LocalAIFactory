...
public static class DataSeeder
{
    ...
    private static void SeedRolesAndWorkflows(ErpDbContext db)
    {
        ...
        var admin = new AppUser { Username = "admin", FullName = "System Administrator", PasswordHash = PasswordHasher.Hash("Admin#12345") };
        var alice = new AppUser { Username = "alice", FullName = "Alice (Sales)", PasswordHash = PasswordHasher.Hash("Alice#12345") };
        var bob = new AppUser { Username = "bob", FullName = "Bob (Accounts Manager)", PasswordHash = PasswordHasher.Hash("Bob#12345") };
        db.AppUsers.AddRange(admin, alice, bob);
        db.SaveChanges();

        // Assign roles to the seeded users so real login carries meaningful permissions.
        int RoleId(string n) => db.AppRoles.First(r => r.Name == n).Id;
        db.AppUserRoles.AddRange(
            new AppUserRole { AppUserId = admin.Id, AppRoleId = RoleId("System Manager") },
            new AppUserRole { AppUserId = admin.Id, AppRoleId = RoleId("Accounts Manager") },
            new AppUserRole { AppUserId = alice.Id, AppRoleId = RoleId("Sales User") },
            new AppUserRole { AppUserId = bob.Id, AppRoleId = RoleId("Accounts Manager") },
            new AppUserRole { AppUserId = bob.Id, AppRoleId = RoleId("Accounts User") });
        db.SaveChanges();
