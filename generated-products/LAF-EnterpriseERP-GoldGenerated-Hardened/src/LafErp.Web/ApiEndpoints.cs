...
public static class ApiEndpoints
{
    public static void Map(WebApplication app)
    {
        ...
        api.MapPost("/catalog/deliverynotes", ([Microsoft.AspNetCore.Mvc.FromServices] CatalogCrudService<DeliveryNote> svc, [Microsoft.AspNetCore.Mvc.FromBody] DeliveryNote e) => { try { return Results.Created("/api/catalog/deliverynotes", svc.Create(e)); } catch (LafErp.Core.DomainException ex) { return Results.BadRequest(new { error = ex.Message }); } });
        api.MapGet("/catalog/creditnotes", (ErpDbContext db) => Results.Ok(db.Set<CreditNote>().OrderByDescending(x => x.Id).ToList()));
        api.MapPost("/catalog/creditnotes", ([Microsoft.AspNetCore.Mvc.FromServices] CatalogCrudService<CreditNote> svc, [Microsoft.AspNetCore.Mvc.FromBody] CreditNote e) => { try { return Results.Created("/api/catalog/creditnotes", svc.Create(e)); } catch (LafErp.Core.DomainException ex) { return Results.BadRequest(new { error = ex.Message }); } });
        api.MapGet("/catalog/purchasereceipts", (ErpDbContext db) => Results.Ok(db.Set<PurchaseReceipt>().OrderByDescending(x => x.Id).ToList()));
        api.MapPost("/catalog/purchasereceipts", ([Microsoft.AspNetCore.Mvc.FromServices] CatalogCrudService<PurchaseReceipt> svc, [Microsoft.AspNetCore.Mvc.FromBody] PurchaseReceipt e) => { try { return Results.Created("/api/catalog/purchasereceipts", svc.Create(e)); } catch (LafErp.Core.DomainException ex) { return Results.BadRequest(new { error = ex.Message }); } });
        api.MapGet("/catalog/materialrequests", (ErpDbContext db) => Results.Ok(db.Set<MaterialRequest>().OrderByDescending(x => x.Id).ToList()));
        api.MapPost("/catalog/materialrequests", ([Microsoft.AspNetCore.Mvc.FromServices] CatalogCrudService<MaterialRequest> svc, [Microsoft.AspNetCore.Mvc.FromBody] MaterialRequest e) => { try { return Results.Created("/api/catalog/materialrequests", svc.Create(e)); } catch (LafErp.Core.DomainException ex) { return Results.BadRequest(new { error = ex.Message }); } });
        api.MapGet("/catalog/debitnotes", (ErpDbContext db) => Results.Ok(db.Set<DebitNote>().OrderByDescending(x => x.Id).ToList()));
        api.MapPost("/catalog/debitnotes", ([Microsoft.AspNetCore.Mvc.FromServices] CatalogCrudService<DebitNote> svc, [Microsoft.AspNetCore.Mvc.FromBody] DebitNote e) => { try { return Results.Created("/api/catalog/debitnotes", svc.Create(e)); } catch (LafErp.Core.DomainException ex) { return Results.BadRequest(new { error = ex.Message }); } });
        api.MapGet("/catalog/stocktransfers", (ErpDbContext db) => Results.Ok(db.Set<StockTransfer>().OrderByDescending(x => x.Id).ToList()));
        api.MapPost("/catalog/stocktransfers", ([Microsoft.AspNetCore.Mvc.FromServices] CatalogCrudService<StockTransfer> svc, [Microsoft.AspNetCore.Mvc.FromBody] StockTransfer e) => { try { return Results.Created("/api/catalog/stocktransfers", svc.Create(e)); } catch (LafErp.Core.DomainException ex) { return Results.BadRequest(new { error = ex.Message }); } });
        api.MapGet("/catalog/stockreconciliations", (ErpDbContext db) => Results.Ok(db.Set<StockReconciliation>().OrderByDescending(x => x.Id).ToList()));
        api.MapPost("/catalog/stockreconciliations", ([Microsoft.AspNetCore.Mvc.FromServices] CatalogCrudService<StockReconciliation> svc, [Microsoft.AspNetCore.Mvc.FromBody] StockReconciliation e) => { try { return Results.Created("/api/catalog/stockreconciliations", svc.Create(e)); } catch (LafErp.Core.DomainException ex) { return Results.BadRequest(new { error = ex.Message }); } });
        api.MapGet("/catalog/pricelists", (ErpDbContext db) => Results.Ok(db.Set<PriceList>().OrderByDescending(x => x.Id).ToList()));
        api.MapPost("/catalog/pricelists", ([Microsoft.AspNetCore.Mvc.FromServices] CatalogCrudService<PriceList> svc, [Microsoft.AspNetCore.Mvc.FromBody] PriceList e) => { try { return Results.Created("/api/catalog/pricelists", svc.Create(e)); } catch (LafErp.Core.DomainException ex) { return Results.BadRequest(new { error = ex.Message }); } });
        ...
        api.MapGet("/catalog/jobcards", (ErpDbContext db) => Results.Ok(db.Set<JobCard>().OrderByDescending(x => x.Id).ToList()));
        api.MapPost("/catalog/jobcards", ([Microsoft.AspNetCore.Mvc.FromServices] CatalogCrudService<JobCard> svc, [Microsoft.AspNetCore.Mvc.FromBody] JobCard e) => { try { return Results.Created("/api/catalog/jobcards", svc.Create(e)); } catch (LafErp.Core.DomainException ex) { return Results.BadRequest(new { error = ex.Message }); } });
        ...
        api.MapGet("/catalog/attendancerecords", (ErpDbContext db) => Results.Ok(db.Set<AttendanceRecord>().OrderByDescending(x => x.Id).ToList()));
        api.MapPost("/catalog/attendancerecords", ([Microsoft.AspNetCore.Mvc.FromServices] CatalogCrudService<AttendanceRecord> svc, [Microsoft.AspNetCore.Mvc.FromBody] AttendanceRecord e) => { try { return Results.Created("/api/catalog/attendancerecords", svc.Create(e)); } catch (LafErp.Core.DomainException ex) { return Results.BadRequest(new { error = ex.Message }); } });
        api.MapGet("/catalog/leaveapplications", (ErpDbContext db) => Results.Ok(db.Set<LeaveApplication>().OrderByDescending(x => x.Id).ToList()));
        api.MapPost("/catalog/leaveapplications", ([Microsoft.AspNetCore.Mvc.FromServices] CatalogCrudService<LeaveApplication> svc, [Microsoft.AspNetCore.Mvc.FromBody] LeaveApplication e) => { try { return Results.Created("/api/catalog/leaveapplications", svc.Create(e)); } catch (LafErp.Core.DomainException ex) { return Results.BadRequest(new { error = ex.Message }); } });
        ...
        api.MapGet("/catalog/timesheets", (ErpDbContext db) => Results.Ok(db.Set<Timesheet>().OrderByDescending(x => x.Id).ToList()));
        api.MapPost("/catalog/timesheets", ([Microsoft.AspNetCore.Mvc.FromServices] CatalogCrudService<Timesheet> svc, [Microsoft.AspNetCore.Mvc.FromBody] Timesheet e) => { try { return Results.Created("/api/catalog/timesheets", svc.Create(e)); } catch (LafErp.Core.DomainException ex) { return Results.BadRequest(new { error = ex.Message }); } });
        ...
        api.MapGet("/health"
        , () => Results.Ok(new { status = "ok", product = "LAF Enterprise ERP GoldGenerated-Hardened" }));
    }
}
