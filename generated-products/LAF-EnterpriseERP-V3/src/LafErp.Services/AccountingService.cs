...
public class AccountingService
{
    ...
    public (decimal Income, decimal Expense, decimal NetProfit) ProfitAndLoss(int companyId)
    {
        var rows = _db.GLEntries.Where(g => g.CompanyId == companyId)
            .Select(g => new { g.Account!.RootType, g.Debit, g.Credit }).AsEnumerable().ToList();
        var income = rows.Where(r => r.RootType == RootType.Income).Sum(r => r.Credit - r.Debit);
        var expense = rows.Where(r => r.RootType == RootType.Expense).Sum(r => r.Debit - r.Credit);
        return (income, expense, income - expense);
    }

    public (decimal Assets, decimal Liabilities, decimal Equity, bool Balanced) BalanceSheet(int companyId)
    {
        var rows = _db.GLEntries.Where(g => g.CompanyId == companyId)
            .Select(g => new { g.Account!.RootType, g.Debit, g.Credit }).AsEnumerable().ToList();
        var assets = rows.Where(r => r.RootType == RootType.Asset).Sum(r => r.Debit - r.Credit);
        var liab = rows.Where(r => r.RootType == RootType.Liability).Sum(r => r.Credit - r.Debit);
        var equity = rows.Where(r => r.RootType == RootType.Equity).Sum(r => r.Credit - r.Debit);
        var income = rows.Where(r => r.RootType == RootType.Income).Sum(r => r.Credit - r.Debit);
        var expense = rows.Where(r => r.RootType == RootType.Expense).Sum(r => r.Debit - r.Credit);
        var retained = income - expense;
        return (assets, liab, equity, Math.Round(assets, 4) == Math.Round(liab + equity + retained, 4));
    }
